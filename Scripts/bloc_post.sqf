...
// road colouser base
{
	if(getPos _x inArea [pos_base, 2500, 2500, 45, false])then{_list_roads = _list_roads - [_x]}
} forEach _list_roads;



// filter from small static
_static_weapon_arry_bloc_post = static_enemy_weapon_arry;

for "_i" from 0 to (count static_enemy_weapon_arry) - 1 do 
{
	_jeep = static_enemy_weapon_arry select _i createVehicle [0,0,0];
	if(((_jeep call BIS_fnc_boundingBoxDimensions) select 0) >= 5 or ((_jeep call BIS_fnc_boundingBoxDimensions) select 1) >= 5 or ((_jeep call BIS_fnc_boundingBoxDimensions) select 2) >= 5)then{_static_weapon_arry_bloc_post = _static_weapon_arry_bloc_post - [static_enemy_weapon_arry select _i]};
	deleteVehicle _jeep;
	sleep 1;
};
...
[
	...
	[selectRandom _static_weapon_arry_bloc_post,[5.25977,3.68164,3.2],323.741,1,0,[-1.94478,0.2354],"","",true,false], 
	["Land_DragonsTeeth_01_1x1_old_F",[-2.7998,-7.33301,0],294.645,1,0,[0,0],"","",true,false], 
	[selectRandom _static_weapon_arry_bloc_post,[-6.59619,-2.86523,3.2],146.238,1,0,[-1.94481,0.235079],"","",true,false], 
...
for "_i" from 0 to _count_bloc_post do 
{
	...
	_unit = _group_defend createUnit [selectRandom inf_enemy_missions_arry, getPos _selected_road, [], 0, "FORM"];
