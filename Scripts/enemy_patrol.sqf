...
waitUntil{
	...
	if(count _count_vehicle <= count_enemy_vehicle_patrol)then{		
		...
		_mission_arry_vehicle append car_enemy_mission_arry;
		_mission_arry_vehicle append hevy_enemy_vehicle_arry;
		_mission_arry_vehicle append enemy_anti_air_vehicle_arry;
		_mission_arry_vehicle append heli_enemy_vehecle_arry;
		_mission_arry_vehicle append enemy_plane_vehecle_arry;
		...
		private _pos_from_vehicle = [_locationPos, 100, 500, 15, 0, 0.3, 0] call BIS_fnc_findSafePos;
		...
		{
			_unit = group ((_vehicle select 1)select 0) createUnit [selectRandom inf_enemy_missions_arry, [0,0,0], [], 0, "FORM"];
